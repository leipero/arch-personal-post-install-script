...
#Packages / Paketi
sudo pacman -S --noconfirm git ufw ffmpegthumbnailer gst-libav gst-plugins-base gst-plugins-good gtk-engine-murrine ntfs-3g p7zip unrar youtube-dl mpv file-roller xorg-fonts-type1 acpid dosfstools gparted ttf-freefont ttf-dejavu ttf-sazanami ttf-fireflysung noto-fonts-emoji xorg-xlsfonts transmission-gtk chromium obs-studio wine-staging wine-nine materia-gtk-theme
...
CLUTTER_VBLANK=none" | sudo tee -a /etc/environment
...
echo "[main]
dns=systemd-resolved" | sudo tee -a /etc/NetworkManager/conf.d/dns.conf
...
Exec = /usr/bin/paccache -rk0" | sudo tee -a /usr/share/libalpm/hooks/cleanup.hook
